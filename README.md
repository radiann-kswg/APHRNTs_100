...
- Botには危機的な言葉を検知した場合に、会話よりも優先して上記の窓口を案内する仕組みが組み込まれています（**既定で有効**）。
  - この窓口案内はユーザーごとに切り替えられます。Botに「ホットラインの案内はオフにして」と伝えると、以後は番号を並べる前に**傾聴・相談を優先**する応答に変わります（「オンにして」で戻せます）。切り替えは本人が明示的に頼んだときだけ行われ、Botが勝手に無効化することはありません。無効にしていても、命に関わる切迫した危険が明確なときは 119番／110番 を一言添えることがあります。
...
- 記録はMisskeyのユーザーIDごとに分離して保存され、他のユーザーの会話に混ざることはありません。相談窓口案内の有効/無効（`user_preferences`）もユーザーごとに保存されます。
...
- 自分のMisskey投稿から「投稿の様子」（投稿数・深夜帯の投稿数など）を数える機能があります。**既定はオフ**で、自分で「投稿の傾向をオンにして」と頼んだときだけ動きます（下記「[投稿からの傾向集計](#投稿からの傾向集計既定オフオプトイン)」参照）。
...
- **定期実行（実運用）**: 現在はこの相互同期（Claude→Bot・Bot→Claudeの両方向）を本番運用の標準としており、ローカルPCのタスクスケジューラ`APHRNTs100-PullBotDigest`が**毎日03:00と10:00 JST**に`npm run sync:remote`を実行します（10:00の回は、眠前〜起床後の朝一にセンパイがMisskey Botへ報告した内容を確実に取り込むための追加）。これによりヘルスシート由来の記録（`logs/bot-digest.md`）とセンパイのセッション記録（`logs/YYYY-MM-DD.md`）が相互に取得・反映されます（Windowsの設定例・スケジュールの変更方法は[deploy/README.md](./deploy/README.md#ローカルpc側の定期同期windowsタスクスケジューラ)を参照）。Claude Desktopアプリ自体はコードを実行できないため、この定期実行はOS側の仕組みに委ねる必要があります。
...
## 投稿からの傾向集計（既定オフ・オプトイン）

記録が途切れた期間は、自己申告だけでは何も分かりません。その穴を埋める材料として、**自分のMisskey投稿を数えるだけ**の機能があります（設計と守るべき原則は[docs/misskey-post-mood-trend.md](./docs/misskey-post-mood-trend.md)）。

- **既定はオフ**です。Botに「投稿の傾向をオンにして」と頼むまで、投稿は1件も取得されません（`.env` の `POST_ANALYSIS_ENABLED` と `BOT_OWNER_USER_ID` の両方が設定されていることも条件です）。
- 取得するのは**本人の投稿だけ**です。ホームタイムラインは購読しないため、他の人の投稿は構造的に1件も入りません。リプライ先・引用元の本文も見ません。
- 保存するのは**数えた結果だけ**です（投稿数・リプライ数・深夜0〜5時の投稿数・最初/最後の投稿時刻・文字数・CW付き投稿数・添付付き投稿数・1時間あたり最大投稿数）。**投稿本文はデータベースにもログにも残しません。** メンション・URL・カスタム絵文字は数える前に除去します。
- 結果は**聞いたときだけ**返します（「最近の投稿の様子は？」）。Botから投稿を根拠に声をかけることはありません。返ってくるのは素の数値と平常時との差だけで、「メンタルスコア」のような合成値や良し悪しの判定は作りません。
- **いつでも止められます。**「投稿の集計はオフにして」と言えば、それまでに溜めた指標と取得位置をその場で削除します。指標は既定180日で自動的に消えます（`POST_ANALYSIS_METRIC_RETENTION_DAYS`）。
- Botから見えない投稿（フォロワー限定など）は黙って欠落するため、出力には必ず「Botから見えている範囲の集計」であることが添えられます。
...
| 永続化層 | [`src/storage/`](./src/storage/) に13ストア＋スキーマ定義（`schema.sql`）。全ストアにユニットテスト |
| 双方向同期ブリッジ | [`src/bridge/`](./src/bridge/) でMarkdownログ ⇄ SQLite を相互変換。テキストのマーカー区間だけを置換し、手書き内容を壊さない設計 |
| スケジューラ | [`src/scheduler/`](./src/scheduler/) で服薬リマインド・傾向ナッジ・週次サマリ・投稿傾向の日次集計を定期実行（JSTの時刻ゲート付き） |
