...
なお、切断中に届いてしまったメンション・一対一チャットは、Bot本体側の**取りこぼし回収（replay）**が再接続時にREST API（`notes/mentions` / `chat/messages/user-timeline`）で遡って処理する（`src/bot/replay.ts`）。watchdogや自動デプロイによる再起動の間に届いたメッセージも、再起動後の初回接続時に同じ仕組みで回収される。
