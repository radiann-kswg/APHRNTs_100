...
function chatNudgeNote(channel: Channel): string {
  ...
  return "- 公開メンションでの会話が、緊急性の高い危機的兆候ではないが、じっくり向き合いたい込み入った悩み相談やCBT的な深掘りに移ってきたと感じたら、「よければ一対一メッセージで続けよう」のように、Misskeyの一対一チャットへ移ることをそっと提案してよい（強制はしない）。ただし希死念慮・自傷など緊急性の高い兆候がある場合は、この提案よりも安全指針の危機対応（相談窓口案内、または設定が無効なユーザーには傾聴・相談）を必ず優先すること。";
}
