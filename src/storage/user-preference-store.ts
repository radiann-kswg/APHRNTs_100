...
export class UserPreferenceStore {
  ...
  get(userId: string): UserPreferences {
    const row = this.db
      .prepare("SELECT crisis_hotline_enabled FROM user_preferences WHERE user_id = ?")
      .get(userId) as { crisis_hotline_enabled: number } | undefined;
    if (!row) {
      return { ...DEFAULT_USER_PREFERENCES };
    }
    return { crisisHotlineEnabled: row.crisis_hotline_enabled !== 0 };
  }
